...
class DumpGenerator:
    ...
    def __init__(params=[]):
        ...
        DumpGenerator.avoidWikimediaProjects(config=config, other=other)

        print(DumpGenerator.welcome())
        ...
        if other["resume"]:
            ...
            config = DumpGenerator.loadConfig(config=config, configfilename=configfilename)
        else:
            os.mkdir(config["path"])
            DumpGenerator.saveConfig(config=config, configfilename=configfilename)
        ...
        DumpGenerator.saveIndexPHP(config=config, session=other["session"])
        DumpGenerator.saveSpecialVersion(config=config, session=other["session"])
        DumpGenerator.saveSiteInfo(config=config, session=other["session"])
        DumpGenerator.bye()
    ...
    def getPageTitlesAPI(config={}, session=None):
        ...
        titles = []
        namespaces, namespacenames = DumpGenerator.getNamespacesAPI(config=config, session=session)
    ...
    def getPageTitlesScraper(config={}, session=None):
        ...
        titles = []
        namespaces, namespacenames = DumpGenerator.getNamespacesScraper(config=config, session=session)
        for namespace in namespaces:
            print("    Retrieving titles in the namespace", namespace)
            url = "%s?title=Special:Allpages&namespace=%s" % (config["index"], namespace)
            r = session.get(url=url, timeout=30)
            raw = r.text
            raw = DumpGenerator.cleanHTML(raw)
            ...
            for i in m:
                t = DumpGenerator.undoHTMLEntities(text=i.group("title"))
    ...
    def getPageTitles(config={}, session=None):
        ...
        if "api" in config and config["api"]:
            try:
                titles = DumpGenerator.getPageTitlesAPI(config=config, session=session)
            except:
    ...
    def getImageNames(config={}, session=None):
        ...
        images = []
        if "api" in config and config["api"]:
            images = DumpGenerator.getImageNamesAPI(config=config, session=session)
        elif "index" in config and config["index"]:
            images = DumpGenerator.getImageNamesScraper(config=config, session=session)
    ...
    def getXMLHeader(config={}, session=None):
        ...
        if config["xmlrevisions"] and config["api"] and config["api"].endswith("api.php"):
            ...
            try:
                xml = "".join(
                    [
                        x
                        for x in DumpGenerator.getXMLPage(
        ...
        if not re.match(r"\s*<mediawiki", xml):
            if config["xmlrevisions"]:
                ...
                header, config = DumpGenerator.getXMLHeader(config=config, session=session)
            else:
                print("XML export on this wiki is broken, quitting.")
                DumpGenerator.logerror(u"XML export on this wiki is broken, quitting.")
                sys.exit()
        ...
        while not re.search(r"</mediawiki>", xml):
            ...
            if c >= maxretries:
                ...
                if not config["curonly"] and "curonly" not in params:
                    ...
                    return DumpGenerator.getXMLPageCore(
            ...
            try:
                ...
                DumpGenerator.handleStatusCode(r)
                xml = DumpGenerator.fixBOM(r)
    ...
    def getXMLPage(config={}, title="", verbose=True, session=None):
        ...
        xml = DumpGenerator.getXMLPageCore(params=params, config=config, session=session)
        if xml == "":
            raise ExportAbortedError(config["index"])
        if "</page>" not in xml:
        ...
        if not config["curonly"] and re.search(r_timestamp, xml):
            while not truncated and params["offset"]:  # next chunk
                ...
                try:
                    xml2 = DumpGenerator.getXMLPageCore(params=params, config=config, session=session)
                except MemoryError:
    ...
    def generateXMLDump(config={}, titles=[], start=None, session=None):
        ...
        xmlfilename = "%s-%s-%s.xml" % (
            DumpGenerator.domain2prefix(config=config),
            config["date"],
        ...
        if config["xmlrevisions"]:
            ...
            try:
                r_timestamp = "<timestamp>([^<]+)</timestamp>"
                for xml in DumpGenerator.getXMLRevisions(config=config, session=session, start=start):
                    ...
                    xml = DumpGenerator.cleanXML(xml=xml)
            ...
            if start:
                ...
                for i in DumpGenerator.reverse_readline(
            ...
            for title in DumpGenerator.readTitles(config, start):
                ...
                try:
                    for xml in DumpGenerator.getXMLPage(config=config, title=title, session=session):
                        xml = DumpGenerator.cleanXML(xml=xml)
                        xmlfile.write(str(xml))
    ...
    def getXMLRevisions(config={}, session=None, allpages=False, start=None):
        ...
        try:
                ...
                if not config["curonly"]:
                    ...
                    while True:
                        ...
                        for page in arvrequest["query"]["allrevisions"]:
                            yield DumpGenerator.makeXmlFromPage(page)
                    ...
                    while True:
                        ...
                        for revid in revids:
                            ...
                            yield DumpGenerator.makeXmlPageFromRaw(xml)
            ...
            if config["curonly"]:
                ...
                c = 0
                for title in DumpGenerator.readTitles(config, start=start):
                ...
                for titlelist in DumpGenerator.readTitles(config, start=start, batch=False):
                    ...
                    while True:
                        ...
                        for pageid in pages:
                            try:
                                xml = DumpGenerator.makeXmlFromPage(pages[pageid])
                                yield xml
                            except PageMissingError:
                                DumpGenerator.logerror(
                                    config=config,
    ...
    def curateImageURL(config={}, url=""):
        ...
        url = DumpGenerator.undoHTMLEntities(text=url)
    ...
    def getImageNamesScraper(config={}, session=None):
        ...
        while offset:
            ...
            for i in m:
                url = i.group("url")
                url = DumpGenerator.curateImageURL(config=config, url=url)
                filename = re.sub("_", " ", i.group("filename"))
                filename = DumpGenerator.undoHTMLEntities(text=filename)
                filename = urllib.parse.unquote(filename)
                uploader = re.sub("_", " ", i.group("uploader"))
                uploader = DumpGenerator.undoHTMLEntities(text=uploader)
    ...
    def getImageNamesAPI(config={}, session=None):
        ...
        while aifrom:
            ...
            DumpGenerator.handleStatusCode(r)
            jsonimages = DumpGenerator.getJSON(r)
            DumpGenerator.delay(config=config, session=session)
        ...
        for filename, url, uploader in images:
            ...
            try:
                ...
                if (
                    ...
                    xmlfiledesc = DumpGenerator.getXMLFileDesc(
    ...
    def getParameters(params=[]):
        ...
        # General params
        parser.add_argument("-v", "--version", action="version", version=DumpGenerator.getVersion())
        parser.add_argument(
        ...
        if args.wiki:
            if args.get_wiki_engine:
                print(DumpGenerator.getWikiEngine(url=args.wiki))
                sys.exit()
        ...
        try:
            ...
            pass
        session.cookies = cj
        session.headers.update({"User-Agent": DumpGenerator.getUserAgent()})
        ...
        if api == "" or index == "":
            if args.wiki:
                if DumpGenerator.getWikiEngine(args.wiki) == "MediaWiki":
                    api2, index2 = DumpGenerator.mwGetAPIAndIndex(args.wiki)
                    if not api:
                        api = api2
                    if not index:
                        index = index2
        ...
        if index and DumpGenerator.checkIndex(index=index, cookies=args.cookies, session=session):
        ...
        if not config["path"]:
            config["path"] = "./%s-%s-wikidump" % (
                DumpGenerator.domain2prefix(config=config, session=session),
                config["date"],
            )
    ...
    def checkRetryAPI(api=None, retries=5, apiclient=False, session=None):
        ...
        while retry < retries:
            try:
                check = DumpGenerator.checkAPI(api, session=session)
                break
            except requests.exceptions.ConnectionError as e:
    ...
    def createNewDump(config={}, other={}):
        ...
        if config["xml"]:
            DumpGenerator.getPageTitles(config=config, session=other["session"])
            titles = DumpGenerator.readTitles(config)
            DumpGenerator.generateXMLDump(config=config, titles=titles, session=other["session"])
            DumpGenerator.checkXMLIntegrity(config=config, titles=titles, session=other["session"])
        if config["images"]:
            images += DumpGenerator.getImageNames(config=config, session=other["session"])
            DumpGenerator.saveImageNames(config=config, images=images, session=other["session"])
            DumpGenerator.generateImageDump(
                config=config, other=other, images=images, session=other["session"]
            )
        if config["logs"]:
            DumpGenerator.saveLogs(config=config, session=other["session"])
    ...
    def resumePreviousDump(config={}, other={}):
        ...
        if config["xml"]:
            titles = DumpGenerator.readTitles(config)
            try:
                lasttitles = DumpGenerator.reverse_readline(
                    "%s/%s-%s-titles.txt"
                    % (
                        config["path"],
                        DumpGenerator.domain2prefix(config=config, session=other["session"]),
                        config["date"],
                    )
                )
            ...
            lastxmltitle = None
            try:
                f = DumpGenerator.reverse_readline(
                    "%s/%s-%s-%s.xml"
                    % (
                        config["path"],
                        DumpGenerator.domain2prefix(config=config, session=other["session"]),
                ...
                for l in f:
                    ...
                    if xmltitle:
                        lastxmltitle = DumpGenerator.undoHTMLEntities(text=xmltitle.group(1))
                        break
            except:
            ...
            if xmliscomplete:
                ...
                titles = DumpGenerator.readTitles(config, start=lastxmltitle)
        ...
        if config["images"]:
            ...
            if lastimage == u"--END--":
                ...
                # so
                images = DumpGenerator.getImageNames(config=config, session=other["session"])
                DumpGenerator.saveImageNames(config=config, images=images)
            # checking images directory
            listdir = []
            try:
                listdir = [
                    n.decode("utf-8") for n in os.listdir("%s/images" % (config["path"]))
                ]
            except:
    ...
    def saveSpecialVersion(config={}, session=None):
        ...
        if os.path.exists("%s/Special:Version.html" % (config["path"])):
            ...
            raw = r.text
            DumpGenerator.delay(config=config, session=session)
            raw = DumpGenerator.removeIP(raw=raw)
    ...
    def saveSiteInfo(config={}, session=None):
        ...
        if config["api"]:
            if os.path.exists("%s/siteinfo.json" % (config["path"])):
                ...
                # MediaWiki 1.11-1.12
                if not "query" in DumpGenerator.getJSON(r):
                    r = session.get(
                        url=config["api"],
                        params={
                            "action": "query",
