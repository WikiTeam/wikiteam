    ...
    while startdate <= enddate:
        ...
        for img_name, img_timestamp, img_user, img_user_text, img_size, img_width, img_height in f:
            if img_timestamp.startswith(startdate.strftime('%Y%m%d')):
                ...
                img_name_ = re.sub(r'"', r'\"', re.sub(r' ', r'_', str(img_name))) # do not use ur'', it is encoded
                original_name_ = re.sub(r'"', r'\"', re.sub(r' ', r'_', str(original_name))) # do not use ur'', it is encoded
                ...
                if len(img_name) > filenamelimit: #truncate filename if it is long
                    img_saved_as = img_name[:filenamelimit] + md5(re.sub(' ', '_', str(img_name))).hexdigest() + '.' + img_name.split('.')[-1]
